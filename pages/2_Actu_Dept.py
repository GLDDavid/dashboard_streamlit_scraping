...
authenticator = stauth.Authenticate(names, usernames, hashed_passwords, '2_Actu_Dept', "abcdef", cookie_expiry_days=0)
...
if authentication_status:
    ...
    french_stopwords = [
        ...
        "comment", "dans", "danne","d", "d'", "de", "des","déj", "dun","dune", "d'un", "d'une", "du", "dû", "dedans", "dehors", "depuis", "deux", "devrait", "doit", "donc", "dont", "dos", "début", "elle", "elles", "en", "entre",
        "encore", "enfin", "essai", "est", "et", "eu", "faire", "fait", "faites","fémin", "fois", "font", "grand", "hors", "ici", "il", "ils", "je","juste", "l", "l'", "la", "le", "les", "leur", "là","léglis",
        ...
        "novembre", "décembre",
        "pari", "loir", "iledefrance", "ledefrance", "doub", "l'ain","val de", "de france", "et loire", "belfort", "lyon", "france", "fêt", "lain", "côteor",
        ...
        "nordtouraine", "sain","homme", "saint", "dindreetloir", "coteauxsurloire", "tourain", "beaumontlouestault", "cyr", "mazièresdetouraine", "saintantoinedurocher", "neuvyleroi", "tours"
    ]
    ...
    st.markdown('<p class="font">Dashboard des actualités selon le(s) Départerment(s)</p>', unsafe_allow_html=True)
    st.sidebar.markdown("# Actu Département")
    ...
    if uploaded_file is not None:
        ...
        if start_date > end_date:
            ...
            media = st.sidebar.multiselect(
                "Sélection du média :",
                options=df['media'].unique(),
                default=df['media'].unique()
                #default=['Unidivers']
                )

            # on va rendre les filtres actif
            df_selection = df.query(
                "departement == @departement & media == @media"
                )


            ###############  SOUS-TITRE ET DATAFRAME  ###############
            st.header("Etude au niveau national")
            ...
            df_dept = set(df_selection['departement'].tolist())
            st.dataframe(df_selection)

            st.subheader("Département : ")
            dept_str = ', '.join(df_dept)
            st.text(dept_str)
            selection_titre = df_selection['titre'].tolist()
            st.dataframe(selection_titre)
            ...
            ############### CALCUL DE SENTIMENT   ###############
            st.header("Les Sentiments sur les titres concernant le département")
